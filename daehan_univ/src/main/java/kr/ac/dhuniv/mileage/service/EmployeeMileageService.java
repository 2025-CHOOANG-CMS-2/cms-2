...
public class EmployeeMileageService {
    ...
    private final CoreCptInfoRepository2 coreCptInfoRepository;
    private final NcsPrgInfoRepository2 ncsPrgInfoRepository;

    // 핵심역량 목록
    public List<CoreCptForMlgDto> getAllCoreCompetencies() {
        List<CoreCptInfo> entities = coreCptInfoRepository.findByParentIsNull();
    ...
    public List<NcsPrgForMlgDto> getAllNcsPrograms() {
        List<NcsPrgInfo> entities = ncsPrgInfoRepository.findAll();
        return entities.stream()
    ...
    public List<CompletedStudentDto> getCompletedStudents(LocalDateTime startDate, LocalDateTime endDate, Long coreCptId, Long programId) {
        return cmpInfoRepository.findCompletedStudentsByFilter(startDate, endDate, coreCptId, programId);
    }
